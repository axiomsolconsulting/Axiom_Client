                    ...
                    <div className="flex-1 overflow-hidden">
                        <div className="flex gap-8 transition-transform duration-300" style={{ transform: `translateX(-${activeService * 200}px)` }}>
                            {services.map((service, index) => (
                                <Link key={service} href={getLink(service)} onClick={() => setActiveService(index)} className={`text-white text-lg font-semibold  whitespace-nowrap px-4 py-2 rounded-md transition-colors ${index === activeService ? "text-blue-400" : "hover:text-blue-400"}`}>
                                    {service}
                                </Link>
                            ))}
                        </div>
